...
export default connect(
    (state) => ({...state.color, isRTL:state.user.isRTL}),
    ...
    const save = (name, nameAr, id) => {
        if (editRow) {
            props.requestEditColor(name, nameAr, editRow.id)
        } else
            props.requestAddColor(name, nameAr)
    }
    ...
    return (
        <div>
            <Modal onClose={() => { closeAddedModal(); reset() }} open={isAdded} title={translate('common.success')} body={`${translate('color.color')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.addedSuccess')}`} />
            <Modal onClose={() => { closeEditedModal(); setEditRow(null); reset() }} open={isEdited} title={translate('common.success')} body={`${translate('color.color')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.editedSuccess')}`} />
