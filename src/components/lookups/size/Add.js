...
export default connect(
    (state) => ({...state.size, isRTL:state.user.isRTL}),
    ...
    const save = (name, nameAr, id) => {
        if (editRow) {
            props.requestEditSize(name, nameAr, editRow.id)
        } else
            props.requestAddSize(name, nameAr)
    }
    ...
    return (
        <div>
            <Modal onClose={() => { closeAddedModal(); reset() }} open={isAdded} title={translate('common.success')} body={`${translate('size.size')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.addedSuccess')}`} />
            <Modal onClose={() => { closeEditedModal(); setEditRow(null); reset() }} open={isEdited} title={translate('common.success')} body={`${translate('size.size')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.editedSuccess')}`} />
