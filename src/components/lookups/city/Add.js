...
export default connect(
    (state) => ({...state.city , isRTL:state.user.isRTL}),
    ...
    const save = (name, nameAr, id) => {
        if (editRow) {
            props.requestEditCity(name, nameAr, editRow.id)
        } else
            props.requestAddCity(name, nameAr)
    }
    ...
    return (
        <div>
            <Modal onClose={() => { closeAddedModal(); reset() }} open={isAdded} title={translate('common.success')} body={`${translate('city.city')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.addedSuccess')}`} />
            <Modal onClose={() => { closeEditedModal(); setEditRow(null); reset() }} open={isEdited} title={translate('common.success')} body={`${translate('city.city')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.editedSuccess')}`} />
