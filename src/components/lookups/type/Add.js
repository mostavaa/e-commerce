...
export default connect(
    (state) => ({...state.type, isRTL:state.user.isRTL}),
    ...
    const save = (name, nameAr, id) => {
        if (editRow) {
            props.requestEditType(name, nameAr, editRow.id)
        } else
            props.requestAddType(name, nameAr)
    }
    ...
    return (
        <div>
            <Modal onClose={() => { closeAddedModal(); reset() }} open={isAdded} title={translate('common.success')} body={`${translate('type.type')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.addedSuccess')}`} />
            <Modal onClose={() => { closeEditedModal(); setEditRow(null); reset() }} open={isEdited} title={translate('common.success')} body={`${translate('type.type')} ${isRTL ? formInputs.nameAr.value : formInputs.name.value} ${translate('common.editedSuccess')}`} />
            <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center' }}>
                ...
                <div>
                    ...
                    {editRow ? (<Button onClick={() => setEditRow(null)} startIcon={<CancelIcon />} variant="outlined" type="secondary">{translate('common.cancel')}</Button>) : null}
                </div>
            </div>
        </div>
    );
